...
test('remote source', async () => {
  ...
  const agent = await orchestrator.startAgent({ botPath: './src/test-agent.ts', env: NODE_ENV });
...
test('browser', async () => {
  ...
  const agent = await orchestrator.startAgent({ botPath: './src/test-agent.ts', env: BROWSER_ENV });
